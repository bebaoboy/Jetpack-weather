...
object Util {
    ...
    fun formatNormalDate(pattern: String, time: Long, unix: Boolean = false): String {
        return SimpleDateFormat(
            pattern,
            Locale.getDefault()
        ).format(if (unix) (time * 1000) else time)
    }
    ...
    fun getWeatherInfo(code: Int): WeatherInfo {
        return when (code) {
            ...
            56, 57 -> WeatherInfo("Freezing", R.drawable.freezing_drizzle)
            61,
            -> WeatherInfo("Rain: S", R.drawable.rain_slight)
            
            63 -> WeatherInfo("Rain: M", R.drawable.rain_heavy)
            65 -> WeatherInfo("Rain: H", R.drawable.rain_heavy)
            66, 67 -> WeatherInfo("Freezing Rain", R.drawable.freezing_rain)
            71 -> WeatherInfo("Snow Slight", R.drawable.snow_fall_slight)
            73 -> WeatherInfo("Snow moderate", R.drawable.snow_fall_slight)
            75 -> WeatherInfo("Snow Heavy", R.drawable.snow_fall)
            77 -> WeatherInfo("Snow grains", R.drawable.snow_fall)
            80, 81, 82 -> WeatherInfo("Rain light", R.drawable.rain_slight)
            85, 86 -> WeatherInfo("Snow slight", R.drawable.snow_fall_slight)
            95, 96, 99 -> WeatherInfo("Storm", R.drawable.thunder_storm)
